...
export default function OrdersList(props: OrderListProps) {
  ...
  return (
    <List
      dense={true}
      sx={{ overflow: 'auto' }}
      id={`orders-list-${props.status.toString()}`}
      ...
      {ordersData ? (
        ordersData.map((order: Order, index: number) => (
          <ListItem key={index + 1}>
            <Accordion
              disabled={props.status === OrderStatus.Delivered || props.status === OrderStatus.Declined}
              expanded={props.status === OrderStatus.Delivered || props.status === OrderStatus.Declined}
              onChange={handleChange(index + 1)}
            >
              <AccordionSummary>
                <Typography sx={{ width: '33%', flexShrink: 0 }}>Order #{index + 1}</Typography>
                <Typography sx={{ color: 'text.secondary' }}>{order.items.length} items</Typography>
              </AccordionSummary>
              <AccordionDetails>
                <Typography>Order ID: {order.orderId}</Typography>
                <Typography>Client ID: {order.clientId}</Typography>
                <Typography>Created: {order.dateCreated}</Typography>
                <Typography>Delivery address: {order.deliveryAddress.country}</Typography>
              </AccordionDetails>
              <AccordionActions>
